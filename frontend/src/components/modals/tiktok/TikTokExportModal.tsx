...
interface TikTokExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerate: (file: File) => Promise<void>;
}

const TikTokExportModal = ({
  isOpen,
  onClose,
  onGenerate,
}: TikTokExportModalProps) => {
  ...
  const handleGenerate = async () => {
    if (!file) {
      alert("Please select a TikTok Excel template.");
      return;
    }
    ...
    try {
      ...
      showProgress("Generating TikTok Export...");
      ...
      console.error(error);
      alert("Failed to generate TikTok export.");
    } finally {
      hideProgress();
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-[600px] rounded-xl bg-white p-6 shadow-xl">
        <h2 className="text-2xl font-semibold">TikTok Export</h2>

        <p className="mt-2 text-gray-500">
          Upload the Excel template exported from TikTok.
        </p>
...
export default TikTokExportModal;
